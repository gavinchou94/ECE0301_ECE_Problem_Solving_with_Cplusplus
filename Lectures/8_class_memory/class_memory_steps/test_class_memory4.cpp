// This is test_class_memory4.cpp
// Changes from test_class_memory3.cpp:
// 1. test copy assignment operator
// 2. verify deep copy
...
TEST_CASE("Test copy constructor/assignment and deep copy", "[safearray]")
{
  ...
  SafeArray arr2(arr);  // copy here
  SafeArray arr3 = arr; // copy here

  arr[0] = 10; // update RHS next
  arr[1] = 40; // update RHS next

  int test[] = {1, 4, 0, 0, 0};
  for (int i = 0; i < size; i++)
  {
    REQUIRE(arr2[i] == test[i]); // test arr2
    REQUIRE(arr3[i] == test[i]); // test arr3
  }
}