...
const RGBPIXEL BLUE = {0x94, 0x35, 0x00};
const RGBPIXEL GOLD = {0x1C, 0xB8, 0xFF};
const RGBPIXEL GRAY = {0xC7, 0xC9, 0xC8};
...
inline RGBIMAGEDATA import_bmp(std::string infile_name) {
    ...
    if (input.fail())
    {
        ...
        return RGBIMAGEDATA{nullptr, 0, 0};
    }
    ...
    return RGBIMAGEDATA{data, bi.biWidth, biHeight};
}
...
inline bool export_bmp(RGBIMAGEDATA im, std::string outfile_name) {
    ...
    _BITMAPFILEHEADER bf = {0x4d42, // type (ascii BM)
                            486, // size in bytes
    ...
    _BITMAPINFOHEADER bi = {40, // size of structure
