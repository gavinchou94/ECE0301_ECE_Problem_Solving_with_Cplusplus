...
{
    check_node_idx(idx);
    // As per Line 15 above, nodes[i] is a Node object
...
double Network::get_component_value(int idx) const
{
    ...
    // As per Line 16 above, components[i] is still a pointer to Component object
    // use *(x).y to access Component methods (that's ->), same as below
...
{
    check_component_idx(comp_idx);
    check_node_idx(node_idx);
